 ...
 */
export class ElevationTilePersistenceCache {
  private static db: IDBDatabase | null = null;
  private static initPromise: Promise<void> | null = null;
  private static readonly DB_NAME = 'drone-simulator-elevation';
  private static readonly DB_VERSION = 1;
  private static readonly STORE_NAME = 'elevationTiles';
  private static readonly TTL_HOURS = 24;
  private static readonly TTL_MS =
    ElevationTilePersistenceCache.TTL_HOURS * 60 * 60 * 1000;
  ...
  private static async _performInitialization(): Promise<void> {
    try {
      ...
      if (!this._isIndexedDBAvailable()) {
        console.warn('IndexedDB not available; persistent tile cache disabled');
        return;
      }
      ...
      if (deletedCount > 0) {
        console.debug(`Cleaned up ${deletedCount} expired tiles from cache`);
      }
    } catch (error) {
      console.warn('Failed to initialize elevation persistence cache:', error);
      this.db = null;
   ...
   */
  static async get(key: string): Promise<ElevationDataTile | null> {
    if (!this.db) {
      return null;
    }
   ...
   */
  static async set(key: string, tile: ElevationDataTile): Promise<void> {
    if (!this.db) {
      return;
    }
    ...
    try {
      ...
      console.warn(`Error caching tile ${key}:`, error);
  ...
  static async delete(key: string): Promise<void> {
    ...
    try {
      ...
      console.warn(`Error deleting tile ${key} from cache:`, error);
    }
  }
  ...
  static async cleanupExpired(): Promise<number> {
    ...
    try {
      ...
      console.warn('Error cleaning up expired tiles:', error);
      return 0;
    }
  }
