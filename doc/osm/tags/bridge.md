...
The drone simulator uses `bridge` tag for Z-ordering and visual indication in `TerrainCanvasRenderer`:

**Typical values:**
- `yes` - Standard bridge (rendered elevated)
- `aqueduct` - Water-carrying bridge (special handling)
- `suspended` - Alternative form of suspension bridges

**Usage in rendering:**
- Bridges are drawn with `layer` tag for proper Z-ordering
- Visual distinction via color or stroke patterns
- Combines with `tunnel` tag for complex grade separations
