...
The drone simulator uses `tree:lined` tags for vegetation rendering:

**Primary usage:**
- `yes` - Standard tree-lined street/path
- `both` - Explicitly both-sides tree-lined

**Rendering:**
- Tree rows rendered via `vegetationMeshConfig.treeRow`
- Spacing: `intervalMeters: 8` between trees
- Crown: `crownRadiusMin: 1.5m` to `crownRadiusMax: 3m`
- Trunk: `trunkHeightMin: 6m` to `trunkHeightMax: 12m`
