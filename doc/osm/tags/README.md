...
Complete listing of all 45 OSM tags used in the drone simulator project.
...
### Enumeration Values (Most tags)
Tags with predefined values from OSM specification. See individual tag documentation for complete value lists.

### Numeric Values
- `building:levels` — Integer floor count
- `building:min_level` — Integer floor number
- `height` — Decimal meters
- `min_height` — Decimal meters
- `roof:height` — Decimal meters
- `lanes` — Integer count
- `gauge` — Integer mm or text
- `level` — Integer floor number
- `diameter_crown` — Decimal meters
- `circumference` — Decimal meters
- `diameter` — Decimal meters

### Color Values
- `building:colour` — Hex color codes or text names
- `roof:colour` — Hex color codes or text names

### Degree Values
- `roof:direction` — Compass bearing (0-360°)
- `roof:orientation` — Compass bearing (0-360°)
...
## Tags by Usage Category

### Building Visualization (11 tags)
Used by BuildingMeshFactory to render 3D structures:
...
### Road Visualization (4 tags)
Used by TerrainCanvasRenderer for road rendering:
- highway, surface, lanes, bridge

### Railway Visualization (2 tags)
Used by TerrainCanvasRenderer for railway rendering:
- railway, gauge

### Water Visualization (3 tags)
Used by TerrainCanvasRenderer for water rendering:
- water, waterway

### Vegetation Visualization (7 tags)
Used by VegetationMeshFactory and TerrainCanvasRenderer:
- natural, leaf_type, leaf_cycle
- diameter_crown, circumference
- landuse, leisure

### Infrastructure Visualization (9 tags)
Used by StructureMeshFactory and TerrainCanvasRenderer:
- man_made, power, aerialway, aeroway, barrier
- material, diameter
- layer, location (for Z-ordering and placement)

### Multi-purpose (4 tags)
Used across multiple visualizations:
- layer — Z-ordering for roads, railways, water
- location — Placement modifiers
- bridge/tunnel — Road attribute modifiers
- level — Building component modifiers
...
| Multi-purpose tags | 4 |
| **Total unique tags** | **45** |

## See Also

- [Master OSM Documentation Index](../README.md)
- [Value Documentation](../values/README.md)
- [Keys Organization](../keys/README.md)
- [src/config.ts](../../../src/config.ts) — Tag value configurations
