...
The drone simulator uses `leaf:type` to assist with vegetation classification (working with `natural`, `leaf:cycle`, and vegetation config):

**Common combinations:**
- `leaf:type=broadleaf` + `leaf:cycle=deciduous` - Deciduous trees
- `leaf:type=broadleaf` + `leaf:cycle=evergreen` - Broadleaf evergreens
- `leaf:type=coniferous` + `leaf:cycle=evergreen` - Coniferous evergreens (default for conifers)
- `leaf:type=coniferous` + `leaf:cycle=deciduous` - Deciduous conifers (rare, e.g., larch)
- `leaf:type=mixed` - Mixed forest composition
