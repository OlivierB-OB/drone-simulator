...
The drone simulator uses `tunnel` tag for Z-ordering and visual distinction in `TerrainCanvasRenderer`:

**Typical values:**
- `yes` - Standard tunnel (rendered below grade)
- `culvert` - Water tunnel (special handling)

**Usage in rendering:**
- Tunnels are drawn with `layer=-1` for below-grade positioning
- Visual distinction via color or opacity changes
- Combines with `layer` tag for proper Z-ordering
