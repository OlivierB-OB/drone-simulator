...
The drone simulator uses `location` tags for feature positioning and Z-ordering:

**Primary values:**
- `underground` - Below-grade features (parking, tunnels)
- `overground` - Elevated features (bridges, platforms)
- `rooftop` - Roof-mounted infrastructure (antennas, solar panels)

**Usage in rendering:**
- Complements `layer` tag for Z-ordering
- Helps distinguish grade-separated features
- Used with `bridge`/`tunnel` for 3D visualization
